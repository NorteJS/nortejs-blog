...
```css title="app/globals.css"
@theme {
  --font-maven: 'Maven Pro', cursive;
  /* ... */
```

Pronto, agora automaticamente o TailwindCSS irá criar a classe `font-maven` a qual podemos utilizar na nossa aplicação.
...
export default function NotFound() {
  return (
    <main className="max-w-7xl mx-auto py-20 text-center">
      <h2 className="text-4xl font-maven font-black ">
...
2. Perceba que a fonte do nosso h2 está com a classe `font-maven`. Isso faz com que seja aplicada nossa fonte `Maven pro` que havíamos configurado.
...
4. Estamos usando também uma função `buttonVariants`. Isso é do `shadcn`. O botão padrão dele exporta essa função que faz com que possamos usar os estilos do botão em outros elementos _html_. Nesse caso estamos usando a estilização em uma tag `<a></a>`
