...
Ao invés de definirmos a função de forma _inline_, vamos colocá-la em um arquivo à parte.

Como não teremos muitas _server actions_, podemos criar um arquivo `actions.ts` dentro da pasta `lib`.

Vamos definir a action `deleteJob()`.

Note que as _server actions_ sempre recebem um `FormData` automaticamente como argumento. De forma que podemos pegar qualquer dado usando o FormData. Se você ainda não está familiarizado com o objeto FormData, [dê uma olhada aqui](https://developer.mozilla.org/pt-BR/docs/Web/API/FormData/FormData).
